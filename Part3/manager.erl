...
send(From, To, Kg) ->
  Ref = uniqueref(now()),
  ets:insert(manager, {Ref, waiting, unassigned, From, To, Kg}),
...
deliver(Loc) ->
  ...
  %Deliveries = ets:select_reverse(manager, [{{'$1', '$2', '$3', '$4', '$5', '$6'}, [{'==', '$5', Loc}], ['$5']}]),
  % Pickups, items waiting from Loc
  Pickups = ets:select_reverse(manager, [{{'$1', '$2', '$3', '$4', '$5', '$6'}, [{'==', '$3', unassigned}, {'==', '$4', Loc}], ['$5']}]),
...
%%--------------------------------------------------------------------
reserve(From, To, Kg) ->
  Reserved = ets:select_reverse(manager, [{{'$1', '$2', '$3', '$4', '$5', '$6'}, [{'==', '$2', waiting}, {'==', '$4', From}, {'==', '$5', To}], ['$$']}]),
...
%%--------------------------------------------------------------------
reserve(From, Kg) ->
  Reserved = ets:select_reverse(manager, [{{'$1', '$2', '$3', '$4', '$5', '$6'}, [{'==', '$2', waiting}, {'==', '$4', From}], ['$$']}]),
  WeightedReserve = weightedReserve(Reserved, Kg),
  updateReserved(WeightedReserve),
  {ok, WeightedReserve}.
...
%%--------------------------------------------------------------------
pick(Ref) ->
  Pick = ets:select(manager, [{{'$1', '$2', '$3', '$4', '$5', '$6'}, [{'==', '$1', Ref}], ['$$']}]), %TODO {'==', '$2', reserved or indepot}
  Length = length(Pick),
  if
...
%%--------------------------------------------------------------------
drop(Ref) ->
  Drop = ets:select(manager, [{{'$1', '$2', '$3', '$4', '$5', '$6'}, [{'==', '$2', intransit}, {'==', '$1', Ref}], ['$$']}]),
  Length = length(Drop),
  if
...
%%--------------------------------------------------------------------
transit(Ref, Loc) ->
  Trans = ets:select(manager, [{{'$1', '$2', '$3', '$4', '$5', '$6'}, [{'==', '$2', intransit}, {'==', '$1', Ref}], ['$$']}]),
  Length = length(Trans),
  if
...
lookup(Ref) ->
  ...
  if
    Length > 0 -> {ok, Result, self(), ?MODULE}; %TODO not quite as per API
    Length =< 0 -> {error, instance}
  end.
...
weightedReserve([], _Ac) -> [].
updateReserved([[Ref, _Status, Pid, From, To, Kg] | T]) ->
...
updateManagerByRef([[Ref, _Status, _Pid, From, To, Kg]], State) ->
  ets:delete(manager, Ref),
  % unfortuantely we cant update bags, only sets,
  ets:insert(manager, {Ref, State, _Pid, From, To, Kg}).

% Utilised by the transit/2 function in the API
updateTransitState([[Ref, _Status, _Pid, _From, To, Kg]], Loc) ->
  ets:delete(manager, Ref),
  % unfortuantely we cant update bags, only sets,
  ets:insert(manager, {Ref, indepot, _Pid, Loc, To, Kg}).
